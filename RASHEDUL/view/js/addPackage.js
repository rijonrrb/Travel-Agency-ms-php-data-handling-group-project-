function validation(val){
	var destination = val.destination.value;
	...
	if(destination===""){
		document.getElementById('destinationError').innerHTML="Field can't be empty";
		flag = false;
	}
	if(duration===""){
...
function ajax(pform) {
	...
	if(isValid){
		var fd = new FormData();
		fd.append("destination",pform.destination.value);
		...
		xhttp.open("POST","../RASHEDUL/controller/addPackageAction.php");
